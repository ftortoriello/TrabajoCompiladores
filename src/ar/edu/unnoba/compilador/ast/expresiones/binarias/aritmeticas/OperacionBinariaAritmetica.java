...
public abstract class OperacionBinariaAritmetica extends OperacionBinaria {
    ...
    public Expresion evaluar() throws ExcepcionTransformer {
        ...
        if (izquierda instanceof Literal) {
            valorIzq = Literal.getNumero(izquierda);
            if (esElementoNeutroIzq(valorIzq)) return derecha;
            if (esElementoNuloIzq(valorIzq)) return Literal.crearNumero(0, tipo);
        }

        Number valorDer;
        if (derecha instanceof Literal) {
            valorDer = Literal.getNumero(derecha);
            if (esElementoNeutroDer(valorDer)) return izquierda;
            if (esElementoNuloDer(valorDer)) return Literal.crearNumero(0, tipo);
        } else {
        ...
        if (izquierda instanceof Literal) {
            ...
            return Literal.crearNumero(calcularResultado(valorIzq, valorDer), tipo);
        }
        ...
        if (subexprIzq instanceof Literal) {
            valorIzq = Literal.getNumero(subexprIzq);
            ...
            setDerecha(Literal.crearNumero(calcularResultado(valorIzq, valorDer), tipo));
            return this;
        }

        // Hacer lo mismo con el hijo derecho
        if (subexprDer instanceof Literal) {
            valorIzq = Literal.getNumero(subexprDer);
